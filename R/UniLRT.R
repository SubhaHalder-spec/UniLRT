...
UniLRT <- function(sample_data, significance_level, h){
  ...
  for (i in 1:num_samples) {
    ...
    V_R_star <- LRT_H1_new(bootstrap_samples, h) / LRT_H0_new(bootstrap_samples)
  ...
  V_R <- LRT_H1_new(sample_data, h) / LRT_H0_new(sample_data)
